    ...
    while x != 350:
        ...
        for x1, y1, x2, y2 in lst:
            ...
            if (x1 <= x and x <= x2):
                ...
                if not AS.search((x1, y1, x2, y2),AS.root):
                    AS.bufferinsert(((x1, y1, x2, y2),"i"))

            else:
                #AS.emptyallbuffers()
                if AS.search((x1, y1, x2, y2),AS.root):
                    AS.bufferinsert(((x1, y1, x2, y2),"d"))

        AS.emptyallbuffers()
        #AS.print_tree(AS.root)
        if len(AS.root.keys)>=1:#if len(AS) >= 1:

            ASlist = []
            AS.inorder(AS.root, ASlist)
            #print(ASlist)
            for x_1, y_1, x_2, y_2 in ASlist:
                for x__1, y__1, x__2, y__2 in ASlist:
